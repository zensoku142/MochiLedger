...
// 按钮行为由 navigation.test.tsx 单独验证，这里只确认应用能启动并显示真实首页。
...
test('可以渲染基础应用和首页', async () => {
  ...
  // 滚动测试标题来自真实首屏，能证明 App 不只是创建了空 Provider。
  const renderedTree = JSON.stringify(renderer!.toJSON());
  expect(renderedTree).toContain('账本滚动测试');

  // 首页通过共享 Screen 组件读取全局背景色；检查真实渲染结果可防止主题变量或页面样式被误改后失去统一底色。
