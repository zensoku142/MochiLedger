...
// ==================== Android 界面窗口 ====================
// Activity（承载手机界面的原生窗口）把 MochiLedger 根组件交给 React Native 显示。
// 页面恢复前必须先安装 Screens 的 Fragment 工厂，否则系统恢复原生导航页面时可能崩溃。
class MainActivity : ReactActivity() {

  // ---------- 根组件连接 ----------
  // 这个名称必须与 app.json 和 index.js 的登记结果一致，否则原生窗口找不到应用界面。
  override fun getMainComponentName(): String = "MochiLedger"
  ...
  // Fabric（React Native 的新界面绘制方式）是否启用由项目原有开关决定，这里不自行改写。
